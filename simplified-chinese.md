...
Setext 风格的标题是由一行文本底下接着一行 `=` 符号（用于一阶标题）或 `-` 符号（用于二阶标题）所构成：
...
标题的文本可以包含行内格式，例如强调（见下方 [行内格式] 一节）。


### Atx 风格标题 ###

Atx 风格的标题是由一到六个 `#` 符号以及一行文本所组成，你可以在文本后面加上任意数量的 `#` 符号。由行首起算的 `#` 符号数量决定了标题的阶层：

    ## A level-two header

    ### A level-three header ###

如同 setext 风格标题，这里的标题文本同样可包含行内格式：

    # A level-one header with a [link](/url) and *emphasis*

**Extension: `blank_before_header`**

原始 markdown 语法在标题之前并不需要预留空白行。Pandoc 则需要（除非标题位于文档最开始的地方）。这是因为以 `#` 符号开头的情况在一般文本段落中相当常见，这会导致非预期的标题。例如下面的例子：
